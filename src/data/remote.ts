...
export async function loadRemoteData(): Promise<void> {
  try {
    const [productsRes, contentRes, settingsRes] = await Promise.all([
      withTimeout((signal) => api<{ products: Product[] }>('/products?includeHidden=1', { signal })),
      withTimeout((signal) => api<RemoteContent>('/content', { signal })),
      withTimeout((signal) => api<{ settings: Record<string, unknown> }>('/settings', { signal })),
    ])
    remote = {
      products: productsRes.products,
      content: { fields: contentRes.fields, brandMedia: contentRes.brandMedia },
